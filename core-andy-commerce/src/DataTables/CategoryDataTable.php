...
class CategoryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $editBtn = "<a href='" . route('vendor.category.edit', [Auth::user()->username, $query->id]) . "' class='btn btn-primary'><i class='far fa-edit'></i></a>";
                $deleteBtn = "<a href='" . route('vendor.category.destroy', [Auth::user()->username, $query->id]) . "' class='btn btn-danger ml-2 delete-item'><i class='far fa-trash-alt'></i></a>";
            ...
            ->addColumn('banner', function ($query) {
                if (empty(!$query->banner)) {
                    return $img = "<img width='200px' alt='category_banner' src='" . asset($query->banner) . "' ></img>";
                }
            })
            ...
            ->rawColumns(['icon', 'action', 'status', 'banner'])
            ->setRowId('id');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('icon')->width(100)->addClass('text-left'), 
            Column::make('banner')->width(300)->addClass('text-left'), 
            Column::make('name')->addClass('text-left'), 
            Column::make('status')->width(200)->addClass('text-left'), 
