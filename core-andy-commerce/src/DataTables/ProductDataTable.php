...
class ProductDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $editBtn = "<a href='" . route('vendor.products.edit', [Auth::user()->username, $query->id]) . "' class='btn btn-primary'><i class='far fa-edit'></i></a>";

                $deleteBtn = "<a href='" . route('vendor.products.destroy', [Auth::user()->username, $query->id]) . "' class='btn btn-danger ml-2 delete-item'><i class='far fa-trash-alt'></i></a>";

                $moreBtn = '
                <div class="dropdown d-inline dropleft">
                    <button class="btn btn-primary dropdown-toggle ml-1" type="button" id="dropdownMenuButton2" 
                            data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <i class="fas fa-cog"></i>
                    </button>
                    <div class="dropdown-menu">
                        <a class="dropdown-item has-icon" href="' .
                    route('vendor.products-image-gallery.index', [Auth::user()->username, 'product' => $query->id]) .
                    '">
    ...
    {
        return [
            Column::make('id')->addClass('text-left')->title('S/L')->width(50),
            Column::make('image')->addClass('text-left')->width(150)->title('Thumbnail'),
            Column::make('name')->addClass('text-left')->title('Product Name'),
            Column::make('price')->addClass('text-left'),
            Column::make('type')->width(100)->addClass('text-left')->title('Tags'),
            Column::make('status')->addClass('text-left')->title('Published'),
