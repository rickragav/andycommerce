...
class ProductImageGalleryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $deleteBtn = "<a href='" . route('vendor.products-image-gallery.destroy', [Auth::user()->username, $query->id]) . "' class='btn btn-danger ml-2 delete-item'><i class='far fa-trash-alt'></i></a>";
                return  $deleteBtn;
            })
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('id')->width(200)->addClass('text-left'),
            Column::make('image')->addClass('text-left'),
